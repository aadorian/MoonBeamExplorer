        ...
        <Grid item container justify="space-between">
          <Grid item key="blockHeight">
            ...
            <ChartCard title={t("Block Height")}>
              ...
              <Typography variant="h4"><AccessAlarmIcon></AccessAlarmIcon> {blockNumber} </Typography>
            </ChartCard>
          </Grid>
          <Grid key="chainId" item>
            <ChartCard title={t("Chain ID")}>
              <Typography variant="h4"><InsertLinkIcon/>{hexToNumber(chainId)}</Typography>
            </ChartCard>
          </Grid>
          {syncing &&
            <div key="syncing">
          ...
          <Grid key="gasPrice" item>
            <ChartCard title={t("Gas Price")}>
              <Typography variant="h4"><LocalAtmIcon/>{weiToGwei(hexToNumber(gasPrice))} Gwei</Typography>
            </ChartCard>
          </Grid>
          <Grid key="hRate" item>
            <ChartCard title={t("Network Hash Rate")}>
              {block &&
                <HashRate block={block} blockTime={config.blockTime}>
                  {(hashRate: any) => <Typography variant="h4"><NetworkCheckIcon/>{hashRate} GH/s</Typography>}
                </HashRate>
              }
            </ChartCard>
          </Grid>
          <Grid key="peers" item>
            <ChartCard title={t("Peers")}>
              <Typography variant="h4"><PersonAddIcon></PersonAddIcon> {hexToNumber(peerCount)}</Typography>
            </ChartCard>
          </Grid>
        </Grid>
      </Grid>
